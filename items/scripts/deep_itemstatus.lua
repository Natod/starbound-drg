...
function update(dt, fireMode, shiftHeld)
  ...
  self.itemStatus.rAmmoPos = vec2.add(vec2.sub(activeItem.ownerAimPosition(), mcontroller.position()), {0.5,-0.5})
  ...
  if self.reloadable then
    ...
    self.itemStatus.lAmmoPos = vec2.add(vec2.sub(activeItem.ownerAimPosition(), mcontroller.position()), {0.5,0.5})
