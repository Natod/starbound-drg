...
function update(dt)
  ...
  world.spawnProjectile("bullet-4",{0,0})
  sb.print("WOW IT REALLY WORKS")
...
function missionSequence()
  ...
  util.wait(4.0)
  util.appendLists(wave, spawnWave("gatespawn", { { entityType = "cultistknight", count = 2 }, { entityType = "cultistarcher", count = 1 } }))
  util.appendLists(wave, spawnWave("midfieldspawn", { { entityType = "cultistknight", count = 2 }, { entityType = "cultistarcher", count = 1 } }))
  while #wave > 0 do
    wave = util.filter(wave, world.entityExists)
    coroutine.yield()
  ...
  local spawnFunctions = {
    ...
    function() return spawnWave("midfieldspawn", { { entityType = "cultistknight", count = 3}, { entityType = "cultistarcher", count = 2 } }) end,
    function() return spawnWave("gatespawn", { { entityType = "cultistknight", count = 3}, { entityType = "cultistarcher", count = 2 } }) end
  }
  for i = 1, 5 do
...
function spawnBallistas()
  ...
  while true do
    ...
    ballistas = spawnWave("ballistaspawn", { { entityType = "ballista" }, { entityType = "cultistknight" } })
    spawnWave("ballistaspawn", { { entityType = "cultistknight" } })
  end
end

function spawnHelis()
  local helis = spawnWave("helispawn", { { entityType = "helicultist", count = 2} })
  while true do
    for i,entityId in pairs(helis) do
      if not world.entityExists(entityId) then
        ...
        local newHelis = spawnWave("helispawn", { { entityType = "helicultist", count = 1} })
...
function spawnWave(spawnPoint, wave)
  ...
  for _,spawn in pairs(wave) do
    for i = 1, (spawn.count or 1) do
      ...
      if contains({"cultistknight", "cultistarcher", "helicultist"}, spawn.entityType) then
      ...
      local entityId
      if spawn.entityType == "cultistknight" or spawn.entityType == "cultistarcher" then
