...
function init()
  self.list = "scrollArea.verticalLayout.mainQuestList"
  local driller = string.format("%s.%s", self.list, widget.addListItem(self.list))
  widget.setText(string.format("%s.lblQuestEntry", driller), "driller")
  sb.logInfo(sb.print(widget.getSize(self.list)))
  widget.setSize(self.list, {150,300})
  widget.setPosition(driller, {0,-10})
  sb.logInfo(sb.print(widget.getSize(self.list)))
  --local gunner = string.format("%s.%s", self.list, widget.addListItem(self.list))
  --widget.setText(string.format("%s.lblQuestEntry", gunner), "gunner")
  --local engi = string.format("%s.%s", self.list, widget.addListItem(self.list))
  --widget.setText(string.format("%s.lblQuestEntry", engi), "engi")
  --local scout = string.format("%s.%s", self.list, widget.addListItem(self.list))
  --widget.setText(string.format("%s.lblQuestEntry", scout), "scout")
  
end

function update(dt)
  --local listItem = string.format("%s.%s", self.list, widget.addListItem(self.list))
end
